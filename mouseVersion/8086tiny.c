...
{
#ifndef NO_GRAPHICS
    // Only process mouse if SDL window exists and is active
    if (!sdl_screen) {
        return;  // No SDL window, ignore mouse input
    }
    
    int mx, my;
    unsigned char sdl_buttons;

    sdl_buttons = SDL_GetMouseState(&mx, &my);
    
    // Map coordinates using consistent mapping function
...
{
	// Initialize internal mouse state with reasonable defaults
	emu_mouse.x = 320;          // Default X position (center of 640x200)
	emu_mouse.y = 100;          // Default Y position (center of 640x200)
	emu_mouse.buttons = 0;      // No buttons pressed
	emu_mouse.visible = 1;      // Mouse cursor visible by default
    ...
    // Initialize mouse coordinate mapping (default to 640x200 guest coordinates, common DOS graphics mode)
...
void main_loop()
{
	...
	for (int i = 0; i < EMSCRIPTEN_INSTRUCTIONS_PER_FRAME && cont_main_loop == 1; i++)
	{
		...
		if (int8_asap && !seg_override_en && !rep_override_en && regs8[FLAG_IF] && !regs8[FLAG_TF])
        {
            ...
            int8_asap = 0;
#ifndef NO_GRAPHICS
            // Poll mouse state from SDL - do this before processing events to ensure current state
            // This ensures mouse position/buttons are up-to-date when INT 33h is called
            emu_mouse_poll_from_host();
            while(SDL_PollEvent(&sdl_event))
                ...
                {
#ifndef NO_GRAPHICS
                    // SDL mouse event capture:
                    // SDL_PollEvent() delivers mouse events from the SDL window.
                    // Motion events update position; button events update button bitmask.
                    // Events are ignored if SDL window is not active (sdl_screen == NULL).
                    if (sdl_screen) {
                        if (sdl_event.type == SDL_MOUSEMOTION) {
                            // Update mouse X/Y position from motion event
                            // Use consistent coordinate mapping function
                            emu_mouse_map_coords(sdl_event.motion.x, sdl_event.motion.y);
                        }
                        else if (sdl_event.type == SDL_MOUSEBUTTONDOWN) {
                            // Update button bitmask on button down
                            if (sdl_event.button.button == SDL_BUTTON_LEFT) {
                                emu_mouse.buttons |= 1;  // Set bit 0 (left button)
                            }
                            else if (sdl_event.button.button == SDL_BUTTON_RIGHT) {
                                emu_mouse.buttons |= 2;  // Set bit 1 (right button)
                            }
                        }
                        else if (sdl_event.type == SDL_MOUSEBUTTONUP) {
                            // Update button bitmask on button up
                            if (sdl_event.button.button == SDL_BUTTON_LEFT) {
                                emu_mouse.buttons &= ~1;  // Clear bit 0 (left button)
                            }
                            else if (sdl_event.button.button == SDL_BUTTON_RIGHT) {
                                emu_mouse.buttons &= ~2;  // Clear bit 1 (right button)
                            }
                        }
                    }
                    // If sdl_screen is NULL, ignore mouse events (window not active)
#endif
                }
#else
            KEYBOARD_DRIVER;
    ...
    if (!(inst_counter % GRAPHICS_UPDATE_DELAY))
    {
        ...
        if (io_ports[0x3B8] & 2)
        {
            ...
            if (is_display_init != 1)
            {
                ...
                // Update mouse guest coordinate space to match actual graphics mode
                // Use GRAPHICS_X and GRAPHICS_Y which are set by the graphics mode
                // This ensures mouse coordinates match the actual screen resolution
