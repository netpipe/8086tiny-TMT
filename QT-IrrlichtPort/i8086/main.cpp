...
extern unsigned char mem[]; // expose global memory
char pc_interrupt(unsigned char interrupt_num); // declare interrupt function
...
public:
	...
	virtual bool OnEvent(const SEvent& event)
	{
        ...
        {
            u16 ascii = event.KeyInput.Char;
            u32 flags = 0;

            if (event.KeyInput.Shift)  flags |= 0x1000;
            if (event.KeyInput.Control) flags |= 0x2000;
            //if (event.KeyInput.Alt)    flags |= 0x800;

            u16 finalKey = 0x400 | flags;

            if (ascii && ascii <= 0x7F)
                finalKey |= ascii;
            else
                finalKey |= event.KeyInput.Key;

            *(short*)(mem + 0x4A6) = finalKey;
            pc_interrupt(7);
            return true;
        }
...
// Use only 320x200 of it when updating pixels and drawing

void updateMonitorTexture(ITexture* texture) {
    u32* pixels = (u32*)texture->lock();
    if (!pixels) return;

    for (int y = 0; y < 200; ++y) {
        for (int x = 0; x < 320; ++x) {
            int i = y * 512 + x;
            u8 cgaByte = mem[0xB8000 + (y * 160 + x / 2)];
            u8 colorIdx = (x % 2 == 0) ? (cgaByte & 0x0F) : (cgaByte >> 4);
            pixels[i] = pixel_colors[colorIdx]; // convert to ARGB here if needed
        }
    }
