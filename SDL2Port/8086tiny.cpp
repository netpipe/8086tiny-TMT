// AlleyCatTest.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
SDL_Texture* textureConsole = NULL;
...
#ifndef NO_GRAPHICS
unsigned int sdl_key_to_ascii(SDL_Keycode sdl_key)
{
	switch (sdl_key)
	{
	...
	case SDLK_KP_0:              return 256; break;
	case SDLK_KP_1:		        return 257; break;
	case SDLK_KP_2:		        return 258; break;
	case SDLK_KP_3:		        return 259; break;
	case SDLK_KP_4:		        return 260; break;
	case SDLK_KP_5:		        return 261; break;
	case SDLK_KP_6:		        return 262; break;
	case SDLK_KP_7:		        return 263; break;
	case SDLK_KP_8:		        return 264; break;
	case SDLK_KP_9:		        return 265; break;
	...
	case SDLK_NUMLOCKCLEAR:		    return 300; break;
	case SDLK_CAPSLOCK:		    return 301; break;
	case SDLK_SCROLLLOCK:	    return 302; break;
...
void set_video_mode()
{
	...
	gWindow = SDL_CreateWindow("8086", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WIDTH, HEIGHT, SDL_WINDOW_SHOWN);
	...
	sdl_screen = SDL_CreateRGBSurface(SDL_SWSURFACE, WIDTH, HEIGHT, 32, rmask, gmask, bmask, amask);
...
void init(int argc, char** argv)
{
	...
	//disk[0] = (*++argv) ? fopen(*argv, "rb") : NULL;
...
void main_loop()
{
		...
		if (int8_asap && !seg_override_en && !rep_override_en && regs8[FLAG_IF] && !regs8[FLAG_TF])
		{
			...
			while (SDL_PollEvent(&sdl_event))
				...
				case SDL_KEYDOWN:
					...
					break;
				case SDL_TEXTINPUT:
					scratch_uint = sdl_event.text.text[0];
	...
	if (!(inst_counter % GRAPHICS_UPDATE_DELAY))
	{
		...
		texture = SDL_CreateTextureFromSurface(gRenderer, sdl_screen);
		//SDL_Flip(sdl_screen);
		SDL_RenderClear(gRenderer);
		SDL_RenderCopy(gRenderer, texture, NULL, &sdl_screen->clip_rect);
		if (consoleMgr.surface && graphicConsole) {
			if (textureConsole) SDL_DestroyTexture(textureConsole);
			textureConsole = SDL_CreateTextureFromSurface(gRenderer, consoleMgr.surface);
			SDL_RenderCopy(gRenderer, textureConsole, NULL, &consoleMgr.surface->clip_rect);
		}
